...
1. **Centered and Bernstein enclosures.** Compare bounds on the same crossing, tangent, near-miss and sparse scenes. Require conservative results under numerical adversaries; measure false positives, early stops, interval counts and total cost. A stronger challenger bounds the scalar polynomial `g(t)=|Δp+Δvt+½Δat²|²−R(t)²` directly, where `R(t)` includes both enclosing radii and uncertainty. For the current model this has degree at most four. Positive Bernstein coefficient lower bounds would certify separation throughout the window, retaining time relationships that coordinate boxes lose. This challenger is not implemented. Better bounds may pay for more arithmetic, but that is an experiment, not an assumption.
