...
The flight panel now exposes **Run structural sham on active vessel** and **Run structural impulse on active vessel**. Each click performs a fresh native-boundary qualification, releases the qualification hooks, then admits an unpacked active vessel with exactly two dynamic rigidbodies and one active `ConfigurableJoint`. The runner first requires ten consecutive physics boundaries with relative linear and angular speed at or below 0.01 in Unity units. On the next boundary it captures its baseline in the owned pre-physics callback, issues the exact float `AddForce` pair only for impulse mode, and records 120 post-physics samples.
