...
`StructuralExperimentReport` v2 defines the bounded native receipt before the vessel-writing bridge exists. It separates receipt validity, run eligibility and scientific qualification; preserves both bodies' raw center-of-mass pose and velocity at every consecutive epoch; and binds derived axial motion back to a full pre-physics baseline event. A complete impulse receipt must carry an exactly-once command witness from that same owned callback, including both returned `AddForce` calls. A sham must prove that neither call occurred. Every response sample retains both native body identities and an exact callback sequence.

Admission evidence binds the run to exactly two dynamic bodies and one enabled mapped joint with no unmapped joints. Eligible runs also require two installed and removed contact sentinels, a complete epoch window and zero observed contacts or joint breaks. A complete capture may remain `provisional-contact-unobserved` only when all contact evidence is explicitly absent. Installed Unity, KSP and plugin identities, the lifecycle qualification receipt and its common fixed timestep all bind the capture. Capture code still cannot assert that the candidate passed its scientific gates.
...
The receipt now enforces the four prerequisites for a runner: an exactly-once impulse command witness; evidence-backed admission of exactly two dynamic bodies and one enabled mapped joint with explicit contact coverage; a raw baseline event bound to the injection boundary; and fixed-timestep equality between the installed boundary qualification and structural run. The next slice is the native writer. It must populate these witnesses from owned callbacks rather than reconstruct them after the fact. Sham and repeated impulse runs remain separate captures and a later aggregate receipt will own scientific comparison.
