...
The `shadow-report` .NET command reports body-weighted position and velocity RMS for observed strategies while retaining source provenance. Position and velocity remain separate decisions, and an unavailable or skipped endpoint produces no winner. These summaries select the closer one-step translational approximation for the captured workload; they do not establish physical correctness, rotation, force completeness, or gameplay speedup.
