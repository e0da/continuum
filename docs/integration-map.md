...
Open integration work includes downstream force ownership, callback determinism, complete checkpoint state, controller export/import and mod-qualified frame/warp transitions. Inspect those when the next implementation boundary needs them, rather than disassembling the entire game speculatively.
