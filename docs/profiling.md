...
Adding `--continuum-physics-warp-pressure` requests KSP's LOW-mode rate index 3 after the settled interval. Capture starts only after three consecutive updates at 4x physics warp with the vessel still unpacked. Failure to enter that exact mode and rate within five real seconds invalidates the run. Every captured frame must retain 4x warp plus stable positive `fixedDeltaSeconds` and `timeScale`; the ordinary path retains 1x. The profile already records universal time and wall boundaries, so `KspContinuum.Pressure` can report achieved warp and fixed-child demand from an actual physics-warp window. The harness returns to rate zero and HIGH mode before exit, and a reset failure invalidates the receipt.

In the first paired station qualification, the same unchanged 196-part checkpoint advanced 4.34 seconds of universal time in 4.3356 wall seconds at 1x, then 18.4 seconds in 4.5845 wall seconds under the physics-warp mode, an achieved rate of 4.0135x. KSP expanded `fixedDeltaSeconds` from 20 ms to 80 ms. The measured Physics and Script FixedUpdate child means totaled 5.4425 ms at 1x and 6.1411 ms at 4x, or 27.21% and 30.71% of the respective current-step wall budget. Applying the 4x run's measured child work to a hypothetical 20 ms target yields 122.82%. That last value is a pressure projection, not an observed fine-step run or evidence that changing the timestep preserves simulation accuracy.
