...
Next: compare a persistent isolated contact-engine world with checkpoint restore, measuring cold-reconstruction differences separately. A live KSP adapter still needs observed callback ordering, exclusive writer ownership and explicit partial-publication recovery. The [interaction-regime contract](interaction-regimes.md) and [trajectory representation experiments](trajectory-representations.md) remain relevant; richer orbital models do not remove the ownership boundary.
