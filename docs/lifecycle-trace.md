...
The trace can arm while the vessel is not yet ready. It starts retaining events only after the same loaded, unpacked, normal-rate orbital vessel topology and body frame have survived three consecutive host `FixedUpdate` observations. An ineligible observation resets that candidate. A topology, scene or main-body change starts a new candidate window and invalidates an active trace. Ordinary floating-origin translations and Krakensbane velocity-frame corrections do neither. The trace accumulates the origin event's first offset argument and adds it back to body centers, positions and force application positions, matching KSP's subtraction of that offset from loaded orbital vessels. It adds the sampled Krakensbane frame velocity back to rigidbody velocity, matching the corresponding subtraction from loaded bodies. Each event records the cumulative translation, origin-event count and raw frame velocity needed to audit those transforms. Arming callbacks are counted separately and are not presented as captured physics evidence. The same 30-second deadline covers arming and capture. Missing or ambiguous native timing stages make registration unavailable immediately.
...
Events retain clocks, main-thread identity, vessel/frame/topology context and raw part force deposits. Available Rigidbody samples include translation-normalized position, quaternion, frame-normalized linear velocity and angular velocity. Force application positions and body centers use the same cumulative translation; forces, rotations and angular velocities are unchanged. Missing bodies have explicitly unavailable pose data. Raw part force deposits remain component observations; gravity, stock aerodynamics, contacts, constraints and direct Rigidbody writes are not reconstructed as a total force.
