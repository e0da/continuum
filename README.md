...
- The first accepted shadow snapshot also retains its mapped public Unity joint graph. A [structural shadow experiment](docs/structural-shadow.md) defines the controlled two-body impulse-response oracle and includes a portable held-out modal fitting kernel; native excitation and trace capture remain the next increment.
