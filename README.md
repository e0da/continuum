...
- A portable [active-physics takeover canary](docs/active-physics-takeover.md) proves exclusive authority, before-image journaling, exact readback, compensated abort, and explicit indeterminate failure without writing to Unity or KSP. Live writer exclusion and recovery remain unqualified.
- An opt-in [bounded live-dynamics canary](docs/live-dynamics-canary.md) replaces the native physics node for one already-cached render-frame batch, advances a contact-free orbital vessel as one six-degree-of-freedom rigid cluster under frozen central gravity, verifies every published body, and restores the exact native node. The source path and portable kernel are verified; an installed stock/candidate comparison remains pending.
